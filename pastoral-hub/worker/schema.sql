-- PAS-Presensi Database Schema for TiDB Cloud Serverless
-- Run this in the TiDB Cloud SQL Editor, or the worker will auto-create tables on first setup.
...
-- Data migration: copy old is_active → is_active_rh + is_active_im, then drop old column
-- Unconditional sync — idempotent, handles DEFAULT TRUE overriding old FALSE values
...
-- Update unique key to include presensi_type (run separately if needed for existing DB)
